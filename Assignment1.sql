--1. Retrieve the columns ProductID, Name, Color and ListPrice from the Production.Product table, with no filter.
...
SELECT Name + ' ' + Color AS ProductNames
FROM Production.Product
WHERE Color IS NOT NULL;
...
WHERE (Name LIKE '%Crankarm%' AND Color = 'Black')
    OR (Name LIKE '%Chainring%' AND (Color = 'Silver' or Color = 'Black'));
...
FROM Production.Product
Where  (ListPrice = 0 AND Name LIKE 'Seat%')
	OR (ListPrice = 53.99 AND Name LIKE 'S%')
ORDER BY Name ASC;
...
ORDER BY p.ProductSubcategoryID ASC, p.Color DESC;